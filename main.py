...
from naming import destination_for_movie, destination_for_tv, parse_name_year, sanitize_filename
...
def _series_key(s: str) -> str:
    """
    Entfernt gängige Disc/Season Marker aus einem Anzeigenamen, um pro Serie zu gruppieren.
    """
...
def main() -> None:
    # Basis-Pfade bestimmen (Share ggf. mounten)
    ...
    for (series_base, season_no), discs in tv_groups.items():
        ...
        expected_total_eps: Optional[int] = None
        if tmdb_is_enabled() and season_no is not None:
            # !!! FIX: tmdb_get_season_episode_count hat 3 Parameter (ohne Logger)
            expected_total_eps = tmdb_get_season_episode_count(series_name, year_hint, season_no)
            if expected_total_eps:
                remux_log.info(f"TMDb: erwartete Episoden (S{season_no:02d}): {expected_total_eps}")
            else:
        ...
        for idx, d in enumerate(discs_sorted):
            ...
            remux_log.info("-" * 60)
            remux_log.info(
                f"Disc: {disc_no if disc_no else '?'} | Quelle: {src_path} | Bezeichner: {disp} | Letzte Disc: {is_last_disc}"
            )

            tmp_out = remux_root / "_tmp" / sanitize_filename(
                f"{(series_name or disp)}_S{(season_no or 0):02d}_D{(disc_no or 0):02d}"
            )
            ensure_dir(tmp_out)
            ...
            # Optional: mkv-episode-matcher Hook (auf Season-Ordner)
